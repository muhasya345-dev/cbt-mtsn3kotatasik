...
import { Plus, Trash2, Edit2, FileText, ChevronLeft, Send } from "lucide-react";
...
export function QuestionsPageContent() {
  ...
  const fetchAssignments = useCallback(async () => {
    ...
    } catch { toast.error("Gagal memuat penugasan"); }
    setLoading(false);
  }, []);

  const fetchQuestions = useCallback(async (assignmentId: string) => {
    try {
  ...
  function selectAssignment(a: AssignmentData) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      fetchAssignments(); // refresh question count
    } catch (err) {
  ...
  async function handleSubmitAssignment() {
    if (!selectedAssignment) return;
    if (questions.length === 0) { toast.error("Tambahkan minimal 1 soal sebelum submit"); return; }
    if (!confirm("Submit soal ke admin? Setelah submit, soal tidak bisa diedit lagi.")) return;
    try {
      ...
      toast.success("Soal berhasil disubmit ke admin!");
        ...
        {assignments.length === 0 ? (
          <FadeIn delay={0.1}>
            <Card><CardContent className="py-12 text-center text-muted-foreground">
              ...
              <p>Belum ada penugasan. Hubungi admin untuk mendapatkan penugasan soal.</p>
            </CardContent></Card>
          </FadeIn>
        ) : (
                ...
                >
                  ...
                  <CardContent>
                    <div className="space-y-1 text-sm text-muted-foreground">
                      <p>Event: <span className="text-foreground font-medium">{a.examEventName}</span></p>
                      <p>Kelas: <span className="text-foreground font-medium">{a.className}</span></p>
                      <p>Jumlah Soal: <span className="text-foreground font-medium">{a.questionCount}</span></p>
                    </div>
                  </CardContent>
                </Card>
              </StaggerItem>
  ...
  const canEdit = selectedAssignment.status === "pending";
    ...
    <div className="space-y-6">
      <FadeIn>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            <Button variant="ghost" size="sm" onClick={() => setSelectedAssignment(null)} className="cursor-pointer">
              <ChevronLeft size={16} />
            </Button>
            <div>
            ...
            {canEdit && (
              <>
                ...
                <Button onClick={handleSubmitAssignment} variant="outline" className="border-green-600 text-green-600 hover:bg-green-50 cursor-pointer">
                  <Send size={16} className="mr-2" /> Submit ke Admin
                </Button>
              </>
            )}
          </div>
      ...
      {questions.length === 0 ? (
        <FadeIn delay={0.1}>
          <Card><CardContent className="py-12 text-center text-muted-foreground">
            <FileText size={48} className="mx-auto mb-4 opacity-50" />
            <p>Belum ada soal. Klik &quot;Tambah Soal&quot; untuk mulai membuat soal.</p>
          </CardContent></Card>
        </FadeIn>
      ) : (
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      {q.type === "true_false" && q.correctAnswer && (
                        <p className="mt-1 text-sm text-green-600 font-medium">Jawaban: {q.correctAnswer}</p>
                      )}
                    </div>
                    {canEdit && (
                      <div className="flex gap-1">
                        <Button size="sm" variant="ghost" onClick={() => openEdit(q)} className="cursor-pointer">
                          <Edit2 size={14} />
                        </Button>
                        <Button size="sm" variant="ghost" className="text-red-600 cursor-pointer" onClick={() => handleDelete(q.id)}>
                          <Trash2 size={14} />
                        </Button>
                      </div>
                    )}
